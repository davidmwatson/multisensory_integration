...
"""
Classes and functions for performing video streaming
"""
    ...
    * FrameStim - Thin wrapper around PsychoPy's ImageStim class.  Can be
    ...
    def get_frame(self):
        ...
        frame : numpy array with uint8 datatype or None
            ...
            returned as a numpy array.  If it fails, a warnings is printed
            and the function returns None instead.
        """
        # Get frame
...
class OpenCV_VideoStream(BaseVideoStream):
    ...
    fps - float, optional
        ...
        laptop cameras, and seems to be the practical limit of the uEye
        camera when using OpenCV to do the image acquisition.  If faster
        frame rates are required, consider using a streaming method
        based upon the camera's SDK.
...
class VideoDelayLine(object):
    """
    Class implements a delay line for video frames.  Frames may be appended to
    and returned from the delay line using the .collector() method. Delay line
    will be pre-allocated with blank images so that first few calls will return
    these until the delay line has been filled with actual acquired frames.
    The collector method can be passed as the postproc function to a
    VideoStream class.

    Arguments
    ---------
    delay : float or int, required
        Delay period in specified units.
    cam_res : (width, height) tuple of ints, required
        Resolution of camera frames that will be passed to delay line.
        Necessary for pre-allocating blank images in delay line.
    colour : bool, required
        If True, will assume images will be in a 3 channel colour space
        (e.g. RGB or BGR).  If False, will assume images are simply grayscale.
        Necessary for pre-allocating blank images in delay line.
    inherent_delay : float or int, optional
        If not None, allows specifying of inherent delay (in specified units)
        present in system.  Requested delay period will be adjusted to
        compensate.  For instance, if a 0.3s delay is requested, and the
        inherent delay is set to 0.1s, then the delay line will implement a
        delay of 0.2s in order to achieve 0.3s in reality.  NOTE - the inherent
        real-time delay of a live video stream may be different to the inherent
        relative delay of recorded video against the audio track; if video
        stream is to be recorded, it may be necessary to further compensate for
        the inherent delay of the recording offline.
    delay_units : str, {'seconds' (default) | 'ms' | 'frames'}, optional
        Units for delay and inherent_delay values.  If not 'frames', you will
        also need to specify value for fps argument.
    fps : float, required if delay_units are time based ('seconds' or 'ms')
        Frames per second of camera.
    blank_lum : int, optional
        Integer in range 0-255.  Luminance of the blank images returned by the
        delay line until it has been filled with acquired frames.

    Methods
    -------
    .collector
        Function takes a frame as an argument, and returns a delayed frame
        back. Input frames are appended to the right and output frames are
        popped from the left of the delay line.

    Example usage
    -------------
    Create class instance for a delay line of 0.5 seconds at 30 fps.

    >>> delayline = VideoDelayLine(delay=0.5, fps=30, cam_res=(640,480),
    ...                            colour=True)

    Acquire a frame and append to delay line, get delayed frame back.

    >>> import cv2
    >>> cap = cv2.VideoCapture(0)
    >>> ret, curr_frame = cap.read()
    >>> delay_frame = delayline.collector(curr_frame)

    """
    def __init__(self, delay, cam_res, colour, inherent_delay=0,
                 delay_units='seconds', fps=None, blank_lum=0):
        self.delay = delay
        self.cam_res = cam_res
        self.colour = colour
        self.inherent_delay = inherent_delay
        self.delay_units = delay_units
        if self.delay_units == 's':  # also allow abbreviation for seconds
            self.delay_units = 'seconds'
        self.fps = fps
        self.blank_lum = blank_lum

        # Error check delay_units
        if not self.delay_units in ['seconds','ms','frames']:
            raise ValueError('delay_units must be \'seconds\', \'ms\', or '
                             '\'frames\'')

        # Ensure fps specified if required
        if self.delay_units in ['seconds','ms'] and self.fps is None:
            raise TypeError('fps cannot be None if delay_units are '
                            '\'seconds\' or \'ms\'')

        # Adjust delay to compensate inherent delay if necessary
        if self.inherent_delay:
            self.real_delay = self.delay - self.inherent_delay
        else:
            self.real_delay = self.delay

        if self.real_delay < 0:
            raise ValueError('Delay unachievable given inherent delay')

        # Work out delay in units of frames - depends on delay_units
        if self.delay_units == 'frames':
            # For frames, value already in correct units
            self.delay_frames = self.real_delay
        else:  # seconds or ms
            # Multiply by fps, and also divide by 1000 if units are ms
            self.delay_frames = self.fps * self.real_delay
            if self.delay_units == 'ms':
                self.delay_frames /= 1000

        # Should be integer
        if abs(self.delay_frames - round(self.delay_frames)) > 1e-5:
            raise ValueError('Delay must be divisible by fps')
        self.delay_frames = int(round(self.delay_frames))

        # Create deque buffer, pre-allocated with blank images
        if self.delay_frames > 0:
            dims = [self.delay_frames, self.cam_res[1], self.cam_res[0]]
            if colour:
                dims.append(3) # allow for RGB depth
            blanks = (np.ones(dims) * self.blank_lum).astype(dtype = np.uint8)
            self.frame_buffer = deque(blanks)
        else:
            self.frame_buffer = deque()


    def collector(self, frame):
        """
        Function adds provided frame to right of buffer, and returns first
        frame from left of buffer.  You may pass this method to the
        videostream class's postproc argument.
        """
...
class FrameStim(visual.ImageStim):
    ...
    colour_mode - str {'bgr','bgra','rgb','rgba','mono'} or None, optional
        Set colour mode for image display. If None (default), will default
        to 'mono' if image is 2D, 'bgr' if image is 3D with 3 colour channels,
        or 'bgra' if image is 3D with 4 colour channels.
    **kwargs
    ...
    def __init__(self, win, frame=None, rescale='resize', display_size=None,
                 colour_mode=None, **kwargs):
        # Assign local vars into class
        self.win = win
        self.rescale = rescale
        if isinstance(colour_mode, str):
            self.colour_mode = colour_mode.lower()
        elif colour_mode is None:
            self.colour_mode = None
        else:
            raise ValueError('Invalid argument to \'colour_mode\'')
    ...
    def _createTexture(self, data, id, pixFormat, stim, *args, **kwargs):
        """
        ...
        :params:
            ...
            stim:
                ImageStim instance
            pixFormat, *args, **kwargs:
                pixFormat and other args maintained for compatibility with
                ImageStim, but no longer do anything
        """
        if data is None:
            return

        if data.dtype != np.ubyte:
            raise TypeError('Input array should be uint8')
        ...
        if interpolate:
            ...
            if useShaders:
                ...
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_GENERATE_MIPMAP,
                                   GL.GL_TRUE)
                GL.glTexImage2D(GL.GL_TEXTURE_2D, 0, self._internalFormat,
                                data.shape[1], data.shape[0], 0,
                                self._pixFormat, dataType, texture)
            else:  # use glu
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER,
                                   GL.GL_LINEAR_MIPMAP_NEAREST)
                GL.gluBuild2DMipmaps(GL.GL_TEXTURE_2D, self._internalFormat,
    ...
    def frame(self, img):
        ...
        # Work out colour mode if necessary
        if self.colour_mode is None:
            if img.ndim == 2:
                self.colour_mode = 'mono'
            else:
                if img.shape[2] == 3:
                    self.colour_mode = 'bgr'
                elif img.shape[2] == 4:
                    self.colour_mode = 'bgra'

        # Work out internal and pix formats if necessary
        if self._internalFormat is None:
            if self.colour_mode in ['bgra', 'rgba']:
                self._internalFormat = GL.GL_RGBA
            elif self.colour_mode in ['bgr', 'rgb']:
                self._internalFormat = GL.GL_RGB
            else:
                self._internalFormat = GL.GL_LUMINANCE

        if self._pixFormat is None:
            if self.colour_mode == 'bgr':
                self._pixFormat = GL.GL_BGR
            elif self.colour_mode == 'bgra':
                self._pixFormat = GL.GL_BGRA
            elif self.colour_mode == 'rgb':
                self._pixFormat = GL.GL_RGB
            elif self.colour_mode == 'rgba':
                self._pixFormat = GL.GL_RGBA
            else:
                self._pixFormat = GL.GL_LUMINANCE
